...
- 카카오 지도 API를 처음 활용하면서 비동기 처리와 지도 데이터 연동 과정에서 어려움을 겪었지만, 상태 관리 로직을 개선하여 원하는 기능을 구현할 수 있었습니다.
- 지도 위에 정류장과 노선 정보를 시각화하면서 사용자 입장에서 데이터를 표현하는 방법을 고민할 수 있었습니다.
- API와 프론트엔드 기능을 연결하는 경험을 통해 서비스 개발 전반에 대한 이해를 높일 수 있었습니다.

### 박기현 (ERD 구성 및 테이블 제작, PPT 제작, 발표)
